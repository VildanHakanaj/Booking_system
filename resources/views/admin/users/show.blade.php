                    ...
                    <ul>
                        @if(!empty($user->reasons))
                            @foreach($user->reasons as $reason_to_book)
                                <li id="deactivate" class="w-50 my-2">Title:
                                    {{\App\Reason::where('id', $reason_to_book->reason_id)->pluck('title')[0]}}
                                    -- Status @if($reason_to_book->active == 1) Active @else Not Active @endif

                                    <a href="{{route('reasonToBook.deactivate', [$user->id,
                                    \App\Reason::where('id', $reason_to_book->reason_id)->pluck('id')[0]])}}"
